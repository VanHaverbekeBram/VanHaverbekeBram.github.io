let swatch = null;
let id = 0;
let favourites = {
	swatches: []
};
class  FavSwatch
{
	id;
	rgb;

	constructor(swatch) {
		this.id = id;
		id++;
		this.rgb = swatch;

	}

}

const initialize = () => {
	let sliders = document.getElementsByClassName("slider");
	for (let i = 0; i < sliders.length; i++) {
		// we moeten zowel op het input als het change event reageren,
		// zie http://stackoverflow.com/questions/18544890
		sliders[i].addEventListener("change", update);
		sliders[i].addEventListener("input", update);
	}
	swatch = document.getElementById("swatch");
	let btnsave = document.getElementById("btnSave");
	btnsave.addEventListener("click", save);
	localLoad();
	update();
};

const save = () => {
	console.log("Saving template");
	buildTemplate();
	localSave();
};

const localSave = () =>{
	//Object -> JSON
	let savedFavorites = JSON.stringify(favourites);
	console.log(savedFavorites);
	//JSON -> Object
	//let myFavs = JSON.parse(savedFavorites);

	localStorage.setItem("vives.swatches", savedFavorites);
}

const buildTemplate = (s) =>
{
	//Get template holder
	let template_holder = document.getElementById("templateHolder");

	//Create Template
	let template = document.createElement("div");
	template.classList.add("template");
	if (s === undefined){
		let fav = new FavSwatch(rgb=swatch.style.backgroundColor);
		favourites.swatches.push(fav);
		template.style.backgroundColor = swatch.style.backgroundColor;
		template.id = fav.id.toString();
	}else {
		template.style.backgroundColor = s.rgb;
		template.setAttribute("id", s.id.toString());
	}



	let close = document.createElement("input");
	close.setAttribute("type", "button");
	close.setAttribute("value", "X");
	close.addEventListener("click", deleteTemplate);
	template.appendChild(close);

	template.addEventListener("click", applyTemplate); // Register click event
	template_holder.appendChild(template); // Add to div holder
};

const extractColors = (colors) =>
{
	let vals = colors.substring(colors.indexOf('(') +1, colors.length -1).split(', ');
	return {
		'r': vals[0],
		'g': vals[1],
		'b': vals[2]
	};
};

const applyTemplate = (event) =>
{
	let colors = extractColors(event.target.style.backgroundColor);
	swatch.style.backgroundColor = "rgb("+colors['r']+","+colors['g']+","+colors['b']+")";
};

const deleteTemplate = (event) =>
{
	let template_holder = document.getElementById("templateHolder");
	let chosenTemplate = event.target.parentElement;
	console.log(chosenTemplate.id);
	console.log(parseInt(chosenTemplate.id));
	favourites.swatches = favourites.swatches.filter(x => x.id !== parseInt(chosenTemplate.id));
	template_holder.removeChild(chosenTemplate);
	//chosenTemplate.parentElement.removeChild(chosenTemplate);

	localSave();
	console.log(favourites.swatches);
};

const update = () => {
	let red=document.getElementById("sldRed").value; //input always value
	let green=document.getElementById("sldGreen").value;
	let blue=document.getElementById("sldBlue").value;
	document.getElementById("lblRed").innerHTML=red;
	document.getElementById("lblGreen").innerHTML=green;// html-element innerHTML
	document.getElementById("lblBlue").innerHTML=blue;
	swatch.style.backgroundColor="rgb("+red+","+green+","+blue+")";

};

const localLoad = () => {
	let myFavsJson = localStorage.getItem("vives.swatches");
	if (myFavsJson !== null){
		favourites = JSON.parse(myFavsJson);
		favourites.swatches.forEach(function (s)
		{
			buildTemplate(s);
		});
		id = favourites.swatches.at(-1).id + 1;
	}
	//console.log(myFavsJson);
}


window.addEventListener("load", initialize);